...
+ (CGFloat)getMonthTotalHeight:(NSDate *)date type:(CalendarType)type {
    if (type == CalendarType_Week) {
        return yearMonthH + weeksH + dayCellH +30;
    } else {
        NSInteger rows = [[YXDateHelpObject manager] getRows:date];
        return yearMonthH + weeksH + rows * dayCellH + 30;
    }
...
- (void)settingHeadLabel {
    ...
    _yearMonthL.backgroundColor = [ResourceManager mainColor];
    [self addSubview:_yearMonthL];
    
    

    UIButton *btnRight = [[UIButton alloc] initWithFrame:CGRectMake(ViewW/2 , 0  ,ViewW/2 , yearMonthH)];
    [self addSubview:btnRight];
    [btnRight addTarget:self action:@selector(actionRightPage) forControlEvents:UIControlEventTouchUpInside];
    UIImageView *imgRight = [[UIImageView alloc] initWithFrame:CGRectMake(ViewW-38, 9, 13, 20)];
    [self addSubview:imgRight];
    imgRight.image = [UIImage imageNamed:@"white_right"];
    
    UIButton *btnLeft = [[UIButton alloc] initWithFrame:CGRectMake(0 , 0  ,ViewW/2 , yearMonthH)];
    [self addSubview:btnLeft];
    [btnLeft addTarget:self action:@selector(actionLeftPage) forControlEvents:UIControlEventTouchUpInside];
    btnLeft.backgroundColor = [UIColor clearColor];
    UIImageView *imgLeft = [[UIImageView alloc] initWithFrame:CGRectMake(25 , 9  , 13, 20)];
    [self addSubview:imgLeft];
    imgLeft.image = [UIImage imageNamed:@"white_left"];
    ...
    for (int i = 0; i < 7; i++) {
        ...
        weekL.textColor = [ResourceManager mainColor];
        [self addSubview:weekL];
    }
    
    
    UIImageView *imgLeft1 = [[UIImageView alloc] initWithFrame:CGRectMake(50, yearMonthH-10, 8, 20)];
    [self addSubview:imgLeft1];
    imgLeft1.image = [UIImage imageNamed:@"calendar_juxing"];
    
    UIImageView *imgRight1 = [[UIImageView alloc] initWithFrame:CGRectMake(ViewW-50, yearMonthH-10, 8, 20)];
    [self addSubview:imgRight1];
    imgRight1.image = [UIImage imageNamed:@"calendar_juxing"];
...
- (void)settingScrollView {
    ...
    // 屏蔽掉用户交互
    _scrollV.userInteractionEnabled = NO;
...
- (void)monitorScroll {
    ...
    if (_scrollV.contentOffset.x > 2*ViewW -1) {
        ...
        [self transitionWithType:@"pageCurl" WithSubtype:subtypeString ForView:_middleView];
...
//    for (int i = 0; i < 10; i++) {
//        NSString *dateStr = [NSString stringWithFormat:@"%@-%02d",[[YXDateHelpObject manager] getStrFromDateFormat:@"MM" Date:_currentDate],1 + arc4random()%28];
...
-(void) setEventToView:(NSArray*)arrDay
{
    _middleView.eventArray = arrDay;
}
...
-(void) actionRightPage
{
    ...
    if(_sendSelectMonth)
     {
        NSString *strDate = [[YXDateHelpObject manager] getStrFromDateFormat:@"yyyy-MM" Date:_currentDate];
        _sendSelectMonth(strDate);
     }
...
-(void) actionLeftPage
{
    ...
    NSString *subtypeString;
    subtypeString = kCATransitionFromTop;//kCATransitionFromRight;
