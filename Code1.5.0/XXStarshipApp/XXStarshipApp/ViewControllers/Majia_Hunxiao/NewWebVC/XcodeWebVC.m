...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message
{
    ...
    // 充值  recharge
    if (strParmes &&
        [strParmes isEqualToString:@"recharge"])
     {
        // 游戏充值
        GameRechargeVC *VC = [[GameRechargeVC alloc] init];
    ...
    if (strParmes&&
     ...
     {
        if ([self.jumpType isEqualToString:@"game"]) {
    ...
    if (strParmes&&
        [strParmes containsString:@"backApp"]) {
         [self.navigationController popToRootViewControllerAnimated:YES];
     }
    if (strParmes&&
        [strParmes containsString:@"realName"]) {
    ...
    DDGAFHTTPRequestOperation *operation = [[DDGAFHTTPRequestOperation alloc] initWithURL:[NSString stringWithFormat:@"%@%@",[PDAPI getBaseUrlString],kDDGGetXCode]
                                                                               parameters:nil HTTPCookies:[DDGAccountManager sharedManager].sessionCookiesArray
                                                                                  success:^(DDGAFHTTPRequestOperation *operation, id responseObject){
                                                                                      ...
                                                                                      [self deleteWebCache];
                                                                                      NSURL *url =[NSURL URLWithString:[NSString stringWithFormat:@"%@%@?isApp=1&xcode=%@&uuid=%@&sourceType=tgwios",[PDAPI WXSysRouteAPI2],_homeUrl,[CommonInfo getKey:K_XCODE],[DDGSetting sharedSettings].UUID_MD5]];
