...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self layoutWhiteNaviBarViewWithTitle:@"添加银行卡"];
    ...
    //self.verifBtn.titleLabel.textColor = [ResourceManager mainColor2];
        ...
        if (dicInfo != nil)
         {
            ...
            if (strCardNo&&
                ![strCardNo isEqualToString:@""])
             {
                ...
                _hideIdentity.text = [dicInfo objectForKey:@"cardNo"]; //dicInfo[@"hideCardNo"];
...
-(void) actionTS
{
    ...
    view.image = [UIImage imageNamed:@"bank_iphone"];
...
-(void)verifUrl{
    ...
    params[@"bankCardNo"] = self.cardNumTextField.text;
    params[@"bankName"] = self.YHlabel.text;
    params[@"bankCode"] = _bankCode;
    params[@"mobile"] = self.phoneTextField.text;
    
    DDGAFHTTPRequestOperation *operation = [[DDGAFHTTPRequestOperation alloc] initWithURL:[NSString stringWithFormat:@"%@%@",[PDAPI getBaseUrlString],kDDGBankPreBindCard]
...
-(void)addBackUrl
{
    ...
    params[@"mobile"] = self.phoneTextField.text;
    params[@"smsCode"] = self.verifTextField.text;
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",[PDAPI getBaseUrlString],kDDGBankRealBindCard];    
