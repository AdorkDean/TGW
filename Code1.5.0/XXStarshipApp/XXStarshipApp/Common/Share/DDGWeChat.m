...
-(void) onResp:(BaseResp*)resp
{
    ...
    if(resp.errCode == 0)
    {
        ...
        if ([resp isKindOfClass:[SendMessageToWXResp class]]) {
            ...
            // 分享图片时， 微信的特殊回调
            else if([DDGShareManager  shareManager].block)
             {
                Block_Id tempbolc = [DDGShareManager  shareManager].block;
                tempbolc([NSDictionary dictionaryWithObjectsAndKeys:@"resp.errStr",@"result",@(YES),@"success", nil]);
             }
        }
        // 登录
