...
- (void)timerRepeat {
    int iLeftX = 0;
    int iWdith = self.backView.frame.size.width ;
    UILabel *targetLab = [self.lsLabelArr objectAtIndex:_seconds % self.lsLabelArr.count];
    if (_seconds <= self.lsLabelArr.count) {
        targetLab.frame = CGRectMake(iLeftX, self.backView.frame.size.height, iWdith, self.backView.frame.size.height);
    }
    [UIView animateWithDuration:.75f animations:^{
        targetLab.frame = CGRectMake(iLeftX, self.backView.frame.origin.y, iWdith, self.backView.frame.size.height);
